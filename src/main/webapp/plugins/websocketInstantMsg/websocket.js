...
function toUserMsg(toU){
 ...
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('GZadminqq357049881'+user);
					}
			...
			win.on("close",function(){
				websocket.send('LeaveGZadminqq357049881');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "gzadmin886"+toUser+"gzgzadmin888" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用GZ Admin 即时通讯系统。</div>',
			'<tpl for=".">',
