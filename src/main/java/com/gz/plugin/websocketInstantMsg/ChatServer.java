package com.gz.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("GZadminqq357049881")){
			this.userjoin(message.replaceFirst("GZadminqq357049881", ""),conn);
		}if(null != message && message.startsWith("LeaveGZadminqq357049881")){
			this.userLeave(conn);
		}if(null != message && message.contains("gzadmin886")){
			String toUser = message.substring(message.indexOf("gzadmin886")+10, message.indexOf("gzgzadmin888"));
			message = message.substring(0, message.indexOf("gzadmin886")) +"[私信]  "+ message.substring(message.indexOf("gzgzadmin888")+12, message.length());
